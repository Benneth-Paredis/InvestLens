...
const intervalRangeMap: Record<string, { period1: Date; interval: '1h' | '1d' | '1wk' | '1mo' }> = {
  daily:   { period1: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000),       interval: '1h' },
...
export async function fetchPriceHistory(ticker: string, interval: string): Promise<PriceHistory> {
  ...
  // For hourly data keep HH:MM; for daily/weekly/monthly keep YYYY-MM-DD.
  const isHourly = config.interval === '1h';
  const prices: PricePoint[] = (result.quotes ?? [])
    .filter((q: any) => q.close != null)
    .map((q: any) => ({
      date: isHourly
        ? new Date(q.date).toISOString().slice(0, 16)  // "YYYY-MM-DDTHH:MM"
